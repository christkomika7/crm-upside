...
export const incomeAccountingSchema = z.object({
    ...
    category: z.string().trim().min(1, { error: "La catégorie est requise." }),
    source: z.string().trim().min(1, { error: "La source est requise." }),
    nature: z.string().trim().min(1, { error: "La nature première est requise." }),
    secondNature: z.string().trim().optional(),
    thirdNature: z.string().trim().optional(),
    amount: z.string().trim().min(1, { error: "Le montant est requis." }),
    taxType: z.enum(["HT", "TTC"], { error: "La taxe est requise." }),
    paymentMode: z.enum(["CASH", "CHECK", "BANK"], { error: "Le mode de paiement est requis." }),
    description: z.string({ error: "La description est requise." }).trim().min(20, { error: "La description doit contenir au moins 20 caractères." }),
    documents: z.array(
        z
            .instanceof(File)
            .refine(
