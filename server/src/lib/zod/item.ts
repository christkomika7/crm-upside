    ...
    if (data.type === "UNIT") {
        if (!data.start) {
            ctx.addIssue({
                path: ["start"],
                code: 'custom',
                message: "La date de début est requise pour une unité.",
            });
        }
        ...
        if (!data.end) {
            ctx.addIssue({
                path: ["end"],
                code: 'custom',
                message: "La date de fin est requise pour une unité.",
            });
        }
        ...
        if (data.charges === undefined) {
            ctx.addIssue({
                path: ["charges"],
                code: 'custom',
                message: "Les charges sont requises pour une unité.",
            });
        }
        ...
        if (data.extraCharges === undefined) {
            ctx.addIssue({
                path: ["extraCharges"],
                code: 'custom',
                message: "Les charges supplémentaires sont requises pour une unité.",
            });
        }
    }
});
