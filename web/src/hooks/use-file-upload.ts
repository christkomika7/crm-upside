  ...
  const validateFile = useCallback(
    (file: File | FileMetadata): string | null => {
      ...
      if (accept !== "*") {
        ...
        if (!isAccepted) {
          return `Le fichier "${file instanceof File ? file.name : file.name}" n’est pas dans un format autorisé.`
        }
      }
  ...
  const addFiles = useCallback(
    (newFiles: FileList | File[]) => {
      ...
      if (
        ...
        errors.push(`Vous pouvez importer au maximum ${maxFiles} fichiers.`);
        onError?.(errors)
        setState((prev) => ({ ...prev, errors }))
        return
      }
