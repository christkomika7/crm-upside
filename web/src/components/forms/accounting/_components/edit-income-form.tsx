...
const DEFAULT_VALUES: IncomeAccountingSchemaType = {
    date: new Date(),
    taxType: "HT",
    paymentMode: "CASH",
    category: "",
...
export default function EditIncomeForm({ accountingType, id }: Props) {
    ...
    const toOptions = (data: AccountElement[]) =>
    ...
    const mutation = useMutation({
        mutationFn: ({ incomeId, data }: { data: FormData; incomeId: string }) =>
            crudService.put<FormData, unknown>(`/accounting/income/${incomeId}`, data),
        onSuccess() {
    ...
    useEffect(() => {
        if (!income || !categories || !allocations || !sources) return;
    ...
    }, [income, categories, allocations, sources, reset]);

    useEffect(() => {
        if (!income || !sources) return;
        if (initialized.current.source) return;
        initialized.current.source = true;
        setValue("source", income.sourceId ?? "");
    }, [income, sources, setValue]);

    useEffect(() => {
        if (!income || !natures) return;
        if (initialized.current.nature) return;
        initialized.current.nature = true;
        setValue("nature", income.natureId ?? "");
    }, [income, natures, setValue]);

    useEffect(() => {
        if (!income || !secondNatures) return;
        if (initialized.current.secondNature) return;
        initialized.current.secondNature = true;
        setValue("secondNature", income.secondNatureId ?? "");
    }, [income, secondNatures, setValue]);

    useEffect(() => {
        if (!income || !thirdNatures) return;
        setValue("thirdNature", income.thirdNatureId ?? "");
    }, [income, thirdNatures, setValue]);

    useEffect(() => {
        initialized.current = { base: false, nature: false, secondNature: false, source: false };
    }, [id]);

    function submit(data: IncomeAccountingSchemaType) {
        if (!id) return;
    ...
    return (
        <div className="bg-white rounded-md space-y-4 p-4">
            <Visibility isLoading={isLoading} fallback={<FormSkeleton />}>
                    ...
                    >
                        <div className="grid grid-cols-3 gap-4">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        ...
                                        <FormControl>
                                            <DatePicker date={field.value} setDate={field.onChange} error={!!formState.errors.date} />
                                        </FormControl>
                                        <FormMessage />
                                    </FormItem>
                                )}
                            ...
                            <div className="flex gap-1">
                                ...
                                <FormField
                                    ...
                                    render={({ field }) => (
                                        <FormItem>
                                            <FormLabel className="text-neutral-600 h-3.5"></FormLabel>
                                            <FormControl>
                        ...
                        <div className="flex justify-center">
                            <Button disabled={mutation.isPending} type="submit" variant="action" className="max-w-xl h-11">
                                {mutation.isPending ? (
                                    <span className="flex justify-center items-center"><Spinner /></span>
                                ) : (
                                    "Modifier"
                                )}
                            </Button>
                ...
                <Modal open={openModal === "allocation"} setOpen={(e) => !e && closeModal()} title="Gestion des allocations">
                    <AllocationModal accountingType={accountingType} />
                </Modal>
                <Modal open={openModal === "category"} setOpen={(e) => !e && closeModal()} title="Gestion des catégories">
                    <CategoryModal accountingType={accountingType} />
                </Modal>
                <Modal open={openModal === "source"} setOpen={(e) => !e && closeModal()} title="Gestion des sources">
                    <SourceModal paymentMode={paymentMode} accountingType={accountingType} />
                </Modal>
                <Modal open={openModal === "nature"} setOpen={(e) => !e && closeModal()} title="Gestion des natures">
                    <NatureModal categoryId={category} accountingType={accountingType} />
                </Modal>
                <Modal open={openModal === "secondNature"} setOpen={(e) => !e && closeModal()} title="Gestion des natures secondes">
                    <SecondNatureModal natureId={nature} accountingType={accountingType} />
                </Modal>
                <Modal open={openModal === "thirdNature"} setOpen={(e) => !e && closeModal()} title="Gestion des natures troisièmes">
                    <ThirdNatureModal secondNatureId={secondNature ?? ""} accountingType={accountingType} />
                </Modal>
