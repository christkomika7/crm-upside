...
export default function Share({
    ...
    const { isPending: isGettingQuote, data: document } = useQuery({
    ...
    const mutation = useMutation({
        ...
        onSuccess() {
            ...
            queryClient.invalidateQueries({ queryKey: ["share-quote"] });
            const email = document?.upside.client.email;
            form.reset({
                id: id,
                type: "QUOTE",
                emails: [email!],
    ...
    useEffect(() => {
        if (document) {
            const filename = `Devis ${document.reference}.pdf`;
            const email = document.upside?.client?.email;
            const clientName = document.upside?.client?.name
            ...
            async function convertPdfToFile(document: Document) {
                const pdfData = await renderComponentToPDF(
                    <RecordDocument id={id} title="Devis" type="QUOTE" data={document} />
                    , {
                        padding: 0,
                        margin: 0,
                        quality: 0.98,
    ...
    if (isGettingQuote) return <Spinner />
                ...
                <div className="w-80 shrink-0 flex flex-col gap-3">
                    <div className="rounded-2xl bg-white p-6 flex flex-col gap-6">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-xs font-medium tracking-widest uppercase text-neutral-400">Devis</p>
                        </div>
