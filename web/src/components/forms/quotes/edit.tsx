...
import { quoteSchema, type QuoteSchemaType } from "@/lib/zod/quotes";
...
export default function Edit({ id }: { id: string }) {
    ...
    const { isPending: isGettingQuote, data: quote } = useQuery({
        queryKey: ["quote", id],
        enabled: !!id,
        queryFn: () => apiFetch<Quote>(`/quote/${id}`),
        staleTime: 0,
        gcTime: 0,
    ...
    const mutation = useMutation({
        mutationFn: ({ quoteId, data }: { data: QuoteSchemaType, quoteId: string }) =>
            crudService.put<QuoteSchemaType, any>(`/quote/${quoteId}`, data),
        onSuccess() {
    ...
    async function submit(formData: QuoteSchemaType) {
        const { success, data } = quoteSchema.safeParse(formData);
        if (success && quote?.id) {
            mutation.mutate({ data, quoteId: quote.id });
        }
    }
    ...
    return (
        <div className="bg-white rounded-md space-y-4 p-4">
            <h2 className="font-medium">Modification du devis</h2>
