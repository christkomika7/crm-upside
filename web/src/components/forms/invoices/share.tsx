...
export default function Share({
    ...
    const { isPending: isGettingInvoice, data: document } = useQuery({
    ...
    const mutation = useMutation({
        ...
        onSuccess() {
            ...
            queryClient.invalidateQueries({ queryKey: ["share-invoice"] });
    ...
    useEffect(() => {
        if (document) {
            const filename = `Facture ${document.reference}.pdf`;
            const email = document.upside?.client?.email;
    ...
    if (isGettingInvoice) return <Spinner />
                ...
                <div className="w-80 shrink-0 flex flex-col gap-3">
                    <div className="rounded-2xl bg-white p-6 flex flex-col gap-6">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-xs font-medium tracking-widest uppercase text-neutral-400">Facture</p>
                        </div>
