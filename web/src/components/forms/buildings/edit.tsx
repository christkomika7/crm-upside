...
import type { Building, BuildingClient } from "@/types/building";
...
export default function EditBuilding({ id }: EditBuildingProps) {
    const [open, setOpen] = useState(false);

    const { isPending: isLoadingLotTypes, data: lotTypeOptions } = useQuery({
        queryKey: ["lot-types"],
    ...
    const { isPending: isLoadingBuilding, data: building } = useQuery<BuildingClient>({
        queryKey: ["building", id],
        queryFn: async () => {
            const data = await apiFetch<Building>(`/building/${id}`);
            return {
                ...data,
                photos: data.photos?.length
                    ? await Promise.all(data.photos.map((url) => urlToFile(url)))
                    : [],

                deeds: data.deeds?.length
                    ? await Promise.all(data.deeds.map((url) => urlToFile(url)))
                    : [],
    ...
    const mutation = useMutation({
        mutationFn: ({ buildingId, data }: { data: FormData, buildingId: string }) =>
            crudService.put<FormData, any>(`/building/${buildingId}`, data),
        onSuccess() {
    ...
    useEffect(() => {
        if (building) {
            form.reset({
                ...
                waterBorehole: building.waterBorehole,
                gym: building.gym,
                garden: building.garden,
                map: building.map,
                status: building.status,
                photos: building.photos,
                deeds: building.deeds,
    ...
    async function submit(formData: BuildingSchemaType) {
        ...
        if (success && building?.id) {
            ...
            mutation.mutate({ buildingId: building.id, data: form })
        }
    }

    return (
        <div className="bg-white rounded-md space-y-4 p-4">
            <h2 className="font-medium">Modifier les informations du bâtiment</h2>
            <Activity mode={isLoadingBuilding ? 'visible' : "hidden"}>
                <Spinner />
            </Activity>
            <Form {...form}>
                <form
                                ...
                                <FormItem >
                                    ...
                                    <FormControl>
                                        <MultipleSelector
                                            ...
                                            isGettingData={isLoadingLotTypes}
                                            onChange={(options) => {
